// ============================================================
// TravelVlog — travel-vlogs-grid.js
// Custom Element: <travel-vlogs-grid>
//
// Anasayfada "Tüm vlogları keşfet" ızgarası — travel-explore-grid.js
// (destinasyonlar) ile BİREBİR AYNI görsel stil (font, boyut, renk,
// spacing) — içerik onaylı (Approved) Vlogs kayıtlarından geliyor.
//
// Wix Velo masterPage.js tarafından gönderilmesi beklenen
// mesaj tipi: VLOGS_GRID_UPDATE
//
// payload:
// {
//   items: [
//     {
//       title,             // string
//       link,              // string — vlogLink(item)
//       heroImage,         // string — ZATEN wix:image://'dan çevrilmiş gerçek URL
//       destinationName    // string
//     },
//     ...
//   ],
//   viewAllLink: string   // /vlogs-all sayfasının tam adresi
// }
//
// travel-explore-grid.js ile aynı iki kanalı kullanır:
// data-vlogs-grid attribute'u veya postMessage.
// ============================================================

class TravelVlogsGrid extends HTMLElement {

  static get observedAttributes() {
    return ['data-vlogs-grid'];
  }
  ...
  connectedCallback() {
    ...
    const initialData = this.getAttribute('data-vlogs-grid');
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name === 'data-vlogs-grid' && newValue && newValue !== oldValue) {
  ...
  _onElementMessage(event) {
    ...
    if (message && message.type === 'VLOGS_GRID_UPDATE') {
      this._apply(message.payload);
    }
  }

  _onWindowMessage(event) {
    if (event.data && event.data.type === 'VLOGS_GRID_UPDATE') {
  ...
  _apply(raw) {
    try {
      ...
      this._data = {
        ...
        viewAllLink: data.viewAllLink || '/vlogs-all'
      };
      ...
      console.error('TravelVlogsGrid data error:', error);
    }
  }
  ...
  _build() {
    this.shadowRoot.innerHTML = `
      ...
      <div class="section">
        ...
        <h2 class="heading">Explore all vlogs</h2>

        <div class="grid"></div>

        <div class="view-all-wrap">
          <a class="view-all" href="vlogs-all">
  ...
  _renderData() {
    ...
    if (!items || items.length === 0) {
      ...
      empty.textContent = 'No vlogs yet.';
    ...
    items.forEach((item) => {
      ...
      loc.textContent = item.destinationName || '';
...
if (!customElements.get('travel-vlogs-grid')) {
  customElements.define('travel-vlogs-grid', TravelVlogsGrid);
}
